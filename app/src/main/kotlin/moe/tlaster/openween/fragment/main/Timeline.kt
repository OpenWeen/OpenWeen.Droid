...
class Timeline : WeiboListBase<MessageModel>() {
    ...
    override fun loadMoreOverride(callback: WeiboListBase.Callback<List<MessageModel>>) {
        val jsonCallback = object : WeiboListCallback<MessageListModel>() {
        ...
        if (mGroupID != -1L) {
            Groups.getGroupTimeline(mGroupID.toString(), max_id = mAdapter?.data?.last()?.id!!, count = mLoadCount, callback = jsonCallback)
        } else {
            Home.getTimeline(count = mLoadCount, max_id = mAdapter?.data?.last()?.id!!, callback = jsonCallback)
        }
    }
