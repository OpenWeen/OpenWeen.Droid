 ...
 */
class Comment(override val icon: IIcon = GoogleMaterial.Icon.gmd_comment) : WeiboListBase<CommentModel>() {
    ...
    override fun loadMoreOverride(callback: Callback<List<CommentModel>>) {
        Comments.getCommentToMe(max_id = mAdapter?.data?.last()?.id!!, count = mLoadCount, callBack = object : WeiboListCallback<CommentListModel>() {
    ...
    override fun refreshOverride(callback: Callback<List<CommentModel>>) {
        ...
        Comments.getCommentToMe(count = mLoadCount, callBack = object : WeiboListCallback<CommentListModel>() {
            override fun onResponse(response: CommentListModel?, id: Int) {
                callback.onResponse(response?.comments!!, response?.totalNumber!!)
            }
        })
